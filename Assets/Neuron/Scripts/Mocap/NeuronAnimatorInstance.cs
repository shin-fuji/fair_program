...
/// <summary>
/// Inherited from NeuronInstance. Provides custom methods to apply Neuron motion data to
/// the transform components of the bones bound in the Unity animator component.Needs a
/// humanoid skeleton setup to work properly.
/// </summary>
public class NeuronAnimatorInstance : NeuronInstance
{
	...
	{
        // マッピングされたヒューマンボーン ID の Transform を返します。
        // return transform of mapped humanbone's ID.
	...
	static void SetPosition( Animator animator, HumanBodyBones bone, Vector3 pos )
	{
		...
		if( t != null )
		{
            // "IsNaN(arg)" returns whether "arg" is not a number (NaN).
            if ( !float.IsNaN( pos.x ) && !float.IsNaN( pos.y ) && !float.IsNaN( pos.z ) )
			{
                // 実際には移動して欲しくないので、位置情報だけをコメントアウト
				//t.localPosition = pos;
			}
		}
	}
	...
	static void SetRotation( Animator animator, HumanBodyBones bone, Vector3 rotation )
	{
		...
		if( t != null )
		{
			...
			{
                // pause画面・ゲーム終了画面で動かないようにする
                if (Pausable.pauseGame == false && Pausable.finishGame == false)
                    t.localRotation = rot;
                    //t.localRotation = Quaternion.Euler(rot.eulerAngles);
			}
		}
	}
    // transform.localRotation = muitehosiino.
    // 最初に取るstartEuler = rot.eulerAngles;
    // diffEulerY = rot.eulerAngles.y - startEuler.y;
    // setRot = 
	...
	{
        // The central axis of "NeuronRobot" is the "Hip axis".
        // First, apply Hips position
	...
	{
        // "HumanBodyBones i" represents numbers of HumanBones from Hips to LastBone.
