...
/// <summary>
/// NeuronConnection manages connections with Axis Neuron using BVHDataReader.cs. You
/// can connect to multiple instances of Axis Neuron and each connection will be mapped to a
/// NeuronSource instance.
/// </summary>
namespace Neuron
{
	public static class NeuronConnection
	{
        ...
        // 20170112変更
        // Config画面でAddressが変更された時に用いる変数
        public static string newAddress;
        public static bool changedAdr = false;

        public static NeuronSource Connect( string address, int port, int commandServerPort, SocketType socketType )
        {
            // もしtitle画面のConfigで新しいAddressが入力されていれば、
            // そのAddressを利用する
            if (changedAdr)
            {
                //Debug.Log("New Address is " + newAddress);
                address = newAddress;
            }
